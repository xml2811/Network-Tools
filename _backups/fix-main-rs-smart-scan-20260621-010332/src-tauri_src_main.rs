...
fn scan_local_network() -> Result<Vec<LanHost>, String> {
    ...
    let Some(prefix) = ipv4_prefix_24(&primary_ip) else {
        return Err("Could not calculate local network prefix.".to_string());
    };
...
function Add-HostResult {{
  ...
  foreach ($port in $commonPorts) {{
    try {{
      ...
      $success = $iar.AsyncWaitHandle.WaitOne(180, $false)
...
# 1) ARP cache first: devices Windows already knows
try {{
  arp -a | ForEach-Object {{
    $line = use serde::Serialize;
use serde_json::Value;
use std::net::{IpAddr, TcpStream, ToSocketAddrs};
use std::process::Command;
use std::time::{Duration, Instant};
...
# 2) Fast ping sweep for current /24 only
# V1 safe mode: even if the real network is bigger, scan nearby /24 to avoid freezing.
1..254 | ForEach-Object {{
  $ip = "$prefix.use serde::Serialize;
use serde_json::Value;
use std::net::{IpAddr, TcpStream, ToSocketAddrs};
use std::process::Command;
use std::time::{Duration, Instant};
