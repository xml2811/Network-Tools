...
$items = Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue |
  ...
  ForEach-Object {
    $processName = ''
    try {
      $processName = (Get-Process -Id use serde::Serialize;
use serde_json::Value;
use std::net::{IpAddr, TcpStream, ToSocketAddrs};
use std::process::Command;
use std::time::{Duration, Instant};
...
fn main() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![
            ...
            ping_host,
            test_tcp_port,`r`n            get_local_listening_ports`r`n        ])
        .run(tauri::generate_context!())
        .expect("error while running MPTech Network Tools");
}.OwningProcess -ErrorAction SilentlyContinue).ProcessName
    } catch {
      $processName = ''
    }
    ...
    [PSCustomObject]@{
      Protocol = 'TCP'
      LocalAddress = use serde::Serialize;
use serde_json::Value;
use std::net::{IpAddr, TcpStream, ToSocketAddrs};
use std::process::Command;
use std::time::{Duration, Instant};
